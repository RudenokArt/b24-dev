...
class AwardUsersComponent extends CBitrixComponent {
	...
	public function getDealsSelect () {
		...
		$deals_list = Bitrix\Crm\DealTable::getList([
			'filter' => ['ID' => $deals_id_arr,],
			'select' => ['ID', 'TITLE'],
		])->fetchAll();

		foreach ($arR as $key => $value) {
			foreach ($deals_list as $key1 => $value1) {
				if ($value['title'] == $value1['ID']) {
					$arR[$key]['title'] = $value['title'].' | '.$value1['TITLE'];
				}
			}
		}
	...
	public function getUsers($names = false) {
		...
		if (!$GLOBALS['USER']->isAdmin()) {
			...
			if ($this->users) {
				...
				$award_sort_filter = $params['filter'];
			}
